...
function initStatusMap() {
  statusMap.chapters = el("statusChapters");
  statusMap.verses = el("statusVerses");
  statusMap.search = el("statusSearch");
  statusMap.random = el("statusRandom");
}
...
function restoreState() {
  ...
  if (saved.fontSize) {
    ...
    const range = el("fontSizeRange");
    if (range) range.value = parseFloat(saved.fontSize);
  }
...
function renderVerses(verses, append = false) {
  ...
  verses.forEach((verse) => {
    ...
    card.innerHTML = `
      <div class="d-flex justify-content-between align-items-start mb-2">
        <div>
          <span class="badge bg-light text-dark">${state.currentChapter?.name_arabic} • ${verse.verse_key}</span>
        </div>
  ...
  state.currentVerses = append ? [...state.currentVerses, ...verses] : [...verses];
...
async function loadTranslations() {
  try {
    ...
    if (!hasCurrent) {
      const fallback = data.translations.find((translation) => translation.id === state.translationId);
      if (fallback) {
        const option = document.createElement("option");
...
async function loadSurah(id, append = false) {
  ...
  try {
    ...
    state.versePagination.hasMore = (pagination.total_pages || 0) > (pagination.current_page || 0);
...
async function runGlobalSearch() {
  ...
  try {
    ...
    if (error.name !== "AbortError") {
      console.error(error);
      updateStatus("search", false, "تعذر");
    }
  }
}

async function openVerseFromKey(key) {
  const [chapterId] = key.split(":");
  await loadSurah(parseInt(chapterId, 10));
...
function handleVerseAction(event, verse) {
  ...
  if (action === "play") {
    ...
    navigator.clipboard
      .writeText(`${verse.text_uthmani} ( ${verse.verse_key} )`)
      .then(() => showPlayerStatus("تم النسخ."))
      .catch(() => showPlayerStatus("تعذر النسخ، تحقق من الصلاحيات."));
...
function setCurrentVerseIndex(key) {
  state.currentVerseIndex = state.currentVerses.findIndex((verse) => verse.verse_key === key);
  updatePlayerControls();
}

function updatePlayingVerseStyles() {
  document.querySelectorAll(".verse-card.playing").forEach((card) => card.classList.remove("playing"));
...
function updatePlayerControls() {
  ...
  if (nextBtn) {
    nextBtn.disabled =
      !hasVerses || (!state.versePagination.hasMore && state.currentVerseIndex >= state.currentVerses.length - 1);
  }
}
...
function updatePlayButtonIcon() {
  ...
  const audio = el("globalAudio");
  if (!btn || !audio) return;
  btn.innerHTML = audio.paused ? '<i class="bi bi-play-fill"></i>' : '<i class="bi bi-pause-fill"></i>';
}
...
function applyVerseSearch(value) {
  const normalized = value.trim().toLowerCase();
  document.querySelectorAll(".verse-card").forEach((card) => {
    const text = card.innerText.toLowerCase();
    card.hidden = normalized && !text.includes(normalized);
  });
}
...
function bindEvents() {
  ...
  el("verseSearch")?.addEventListener("input", (event) => applyVerseSearch(event.target.value));
